...
// ---- STATIC METHODS (attach to schema, not model) ----

/**
 * Create a CUSTOMER profile on behalf of another user,
 * only if actor has role manager/agent/admin.
 */
BaseProfileSchema.statics.createCustomerProfileByStaff = async function (
  actorUserId,
  targetUserId,
  data
) {
  ...
  // upsert (one profile per user per role)
  ...
  return await this.create({
    ...
    national_id_front_url: data.national_id_front_url,
    national_id_back_url: data.national_id_back_url,
 ...
 */
BaseProfileSchema.statics.createForSelf = async function (userId, role, data = {}) {
  const User = mongoose.model("User");
  ...
  return await this.create({
    ...
    ...(role === "customer"
      ? {
          loyalty_points: data.loyalty_points,
          // self-created profiles start unverified by default
...
// Compile model AFTER statics & hooks are attached
...
const CustomerProfile = Profile.discriminator(
  ...
  new Schema(
    {
      // Manager/Admin can flip this when they've checked docs, etc.
      verified: { type: Boolean, default: false },
...
const AgentProfile = Profile.discriminator(
  ...
  new Schema(
    {
      // Manager/Admin verification flag for agents
      verified: { type: Boolean, default: false },
